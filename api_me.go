...
// Patch profile
func (r ApiUpdateMeRequest) HandlersUpdateMeRequest(handlersUpdateMeRequest HandlersUpdateMeRequest) ApiUpdateMeRequest {
	r.handlersUpdateMeRequest = &handlersUpdateMeRequest
	return r
}
...
func (a *MeAPIService) UpdateMeExecute(r ApiUpdateMeRequest) (*ModelsUser, *http.Response, error) {
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.handlersUpdateMeRequest == nil {
		return localVarReturnValue, nil, reportError("handlersUpdateMeRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.handlersUpdateMeRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKeyAuth"]; ok {
