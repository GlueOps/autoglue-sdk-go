# DtoCreateClusterRequest

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**ClusterProvider** | Pointer to **string** |  | [optional] 
**DockerImage** | Pointer to **string** |  | [optional] 
**DockerTag** | Pointer to **string** |  | [optional] 
**Name** | Pointer to **string** |  | [optional] 
**Region** | Pointer to **string** |  | [optional] 

## Methods

### NewDtoCreateClusterRequest

`func NewDtoCreateClusterRequest() *DtoCreateClusterRequest`

NewDtoCreateClusterRequest instantiates a new DtoCreateClusterRequest object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewDtoCreateClusterRequestWithDefaults

`func NewDtoCreateClusterRequestWithDefaults() *DtoCreateClusterRequest`

NewDtoCreateClusterRequestWithDefaults instantiates a new DtoCreateClusterRequest object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetClusterProvider

`func (o *DtoCreateClusterRequest) GetClusterProvider() string`

GetClusterProvider returns the ClusterProvider field if non-nil, zero value otherwise.

### GetClusterProviderOk

`func (o *DtoCreateClusterRequest) GetClusterProviderOk() (*string, bool)`

GetClusterProviderOk returns a tuple with the ClusterProvider field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetClusterProvider

`func (o *DtoCreateClusterRequest) SetClusterProvider(v string)`

SetClusterProvider sets ClusterProvider field to given value.

### HasClusterProvider

`func (o *DtoCreateClusterRequest) HasClusterProvider() bool`

HasClusterProvider returns a boolean if a field has been set.

### GetDockerImage

`func (o *DtoCreateClusterRequest) GetDockerImage() string`

GetDockerImage returns the DockerImage field if non-nil, zero value otherwise.

### GetDockerImageOk

`func (o *DtoCreateClusterRequest) GetDockerImageOk() (*string, bool)`

GetDockerImageOk returns a tuple with the DockerImage field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetDockerImage

`func (o *DtoCreateClusterRequest) SetDockerImage(v string)`

SetDockerImage sets DockerImage field to given value.

### HasDockerImage

`func (o *DtoCreateClusterRequest) HasDockerImage() bool`

HasDockerImage returns a boolean if a field has been set.

### GetDockerTag

`func (o *DtoCreateClusterRequest) GetDockerTag() string`

GetDockerTag returns the DockerTag field if non-nil, zero value otherwise.

### GetDockerTagOk

`func (o *DtoCreateClusterRequest) GetDockerTagOk() (*string, bool)`

GetDockerTagOk returns a tuple with the DockerTag field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetDockerTag

`func (o *DtoCreateClusterRequest) SetDockerTag(v string)`

SetDockerTag sets DockerTag field to given value.

### HasDockerTag

`func (o *DtoCreateClusterRequest) HasDockerTag() bool`

HasDockerTag returns a boolean if a field has been set.

### GetName

`func (o *DtoCreateClusterRequest) GetName() string`

GetName returns the Name field if non-nil, zero value otherwise.

### GetNameOk

`func (o *DtoCreateClusterRequest) GetNameOk() (*string, bool)`

GetNameOk returns a tuple with the Name field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetName

`func (o *DtoCreateClusterRequest) SetName(v string)`

SetName sets Name field to given value.

### HasName

`func (o *DtoCreateClusterRequest) HasName() bool`

HasName returns a boolean if a field has been set.

### GetRegion

`func (o *DtoCreateClusterRequest) GetRegion() string`

GetRegion returns the Region field if non-nil, zero value otherwise.

### GetRegionOk

`func (o *DtoCreateClusterRequest) GetRegionOk() (*string, bool)`

GetRegionOk returns a tuple with the Region field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetRegion

`func (o *DtoCreateClusterRequest) SetRegion(v string)`

SetRegion sets Region field to given value.

### HasRegion

`func (o *DtoCreateClusterRequest) HasRegion() bool`

HasRegion returns a boolean if a field has been set.


[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


